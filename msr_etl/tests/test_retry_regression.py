...
class FailRetrySucceedRegressionTestCase(TestCase):
    ...
    def test_fail_then_retry_then_succeed(self, mock_adapter_class, mock_sink_class):
        user = create_test_interactive_user(username="retry_regression_tester")
        job = AsyncJob.objects.create(
            module="msr_etl", job_type="ubr_locations_import", task="x.y", user=user,
        ...
        unit = MsrEtlSyncUnit.objects.create(
            job_uuid=job.id, unit_type=MsrEtlSyncUnit.UnitType.GVH, unit_code="101",
